...
MIN_AREA = 800
MAX_AREA = 3.5 * MIN_AREA#2.5 * MIN_AREA
thres = 42 # min B/W value for threshold
...
#     print("Found %2d blob(s)." % len(blobs))
    ...
    #                cv.putText(frame,str(area),(int(e[0][0]+10),int(e[0][1]-5)), cv.FONT_HERSHEY_SIMPLEX, 0.6,(100,255,20),1)# Add character description
...
video_path =r'C:\Users\inges\OneDrive - UDIT\src\photoneu\videos_ratoncillos\Video_4.MP4'
...
#     print(frame.shape) # 240 x 320
    ...
    _,_, final_frame,_ = analyze_image(i, frame = frame, t_init_resize = t_init_resize)
#    img_out = ruta_carpeta + "/no_labels"+ img_name  
    img_out = ruta_carpeta + r"/no_labels"+ "/vision_spine_2.jpg"  
    write_image(img_out, final_frame)
...
def write_image(img_out, final_frame):
    print(img_out)
...
#    frame_height = int(cap.get(4))
    ...
    convex_out = cv.VideoWriter('no_labels_convex_1.avi',cv.VideoWriter_fourcc('M','J','P','G'), 10, (frame_height,frame_width)) 
    video_out = cv.VideoWriter('no_labels_1.avi',cv.VideoWriter_fourcc('M','J','P','G'), 10, (frame_height,frame_width))
    i = 0
...
def updateDataFrame():
    ...
    print("n_total_mice = " + str(df_tmp[df_tmp["num_mice"] == 3].shape[0]))
